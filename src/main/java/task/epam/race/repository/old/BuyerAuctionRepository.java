...
public class BuyerAuctionRepository implements Repository<BuyerRequest> {

    private static Logger logger = LogManager.getLogger();

    @Language("SQL")
    private static final String ADD_SQL =
            "INSERT INTO `buyer_auction` (`auction_id`, `buyer_id`) VALUES (?, ?)";
    ...
    public void add(BuyerRequest buyerRequest) throws RepositoryException {
        try (Connection connection = ConnectionPool.getInstance().getConnection();
            ...
            throw new RepositoryException("Unable to add buyer auction request", e);
    ...
    public void remove(BuyerRequest buyerRequest) throws RepositoryException {
        try (Connection connection = ConnectionPool.getInstance().getConnection();
            ...
            throw new RepositoryException("Unable to remove buyer auction request", e);
        } catch (ConnectionException e) {
            throw new RepositoryException("Unable to create connection", e);
        }
    }
    ...
    public List<BuyerRequest> query(SqlSpecification spec) throws RepositoryException {
        try (Connection connection = ConnectionPool.getInstance().getConnection();
            ...
            throw new RepositoryException("Unable to query buyer auction on condition", e);
        } catch (ConnectionException e) {
            throw new RepositoryException("Unable to create connection", e);
        }
    }
