...
public final class SqlUserBetConstant {
    ...
    //language=sql
    static final String SQL_USER_BETS_INSERT = "insert into user_bets(user_login, betid, sum, coeff, " +
    ...
    static final String SQL_USER_BETS_SELECT_BY_LOGIN = "SELECT u.user_login, u.betid," +
            ...
            " where user_login = ?";
    //language=sql
    static final String SQL_USER_BETS_SELECT_CURRENT_BY_LOGIN = "SELECT u.user_login, u.betid," +
            "u.sum, h.name, b.position, u.coeff, r.name, r.place, r.time, r.date, u.is_successful " +
            "from user_bets u " +
            "join bets b on(u.betid = b.bet_id) " +
            "join races r on (b.race_id = r.race_id)" +
    ...
    //language=sql
    static final String SQL_USER_BETS_SELECT_NOT_CURRENT_BY_LOGIN = "SELECT u.user_login, u.betid," +
            "u.sum, h.name, b.position, u.coeff, r.name, r.place, r.time, r.date, u.is_successful " +
            "from user_bets u " +
            "join bets b on(u.betid = b.bet_id) " +
            "join races r on (b.race_id = r.race_id)" +
    ...
    static final String SQL_USER_BETS_UPDATE_STATE = "update user_bets set is_current = false, " +
            "is_successful = ? where user_login = ? and betid = ?";
